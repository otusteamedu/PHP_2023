...
class Validator
{
    ...
    public static function validateEmails($emails = []): void
    {
        ...
        if (!is_array($emails)) {
            throw new \Exception('Argument is not array', 400);
        }
        ...
        foreach ($emails as $email) {
            ...
            // Та же самая проверка, но с помощью регулярного выражения по RFC 5322
            // Origin https://emailregex.com/
            ...
            if (!preg_match($email_regex, $email)) {
                throw new \Exception($email . ' is not valid E-mail (RegEx)' . PHP_EOL, 400);
            }
            ...
            if (empty($dns)) {
                throw new \Exception($email . " failed MX-records", 400);
            }
        }
    }
}
