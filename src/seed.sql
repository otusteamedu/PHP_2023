...
INSERT INTO session (hall_id, movie_id, start_time, end_time, price)
VALUES
...
INSERT INTO ticket (session_id, customer_id, seat)
VALUES
    ((SELECT id FROM session WHERE price = 280), (SELECT id FROM customer WHERE first_name = 'Алексей'), 5),
    ((SELECT id FROM session WHERE price = 280), (SELECT id FROM customer WHERE first_name = 'Константин'), 10),
    ((SELECT id FROM session WHERE price = 460), (SELECT id FROM customer WHERE first_name = 'Елена'), 35);
