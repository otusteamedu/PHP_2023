...
CREATE VIEW public.v_important_dates AS
 SELECT a.name,
    c.title,
    to_date(b.val, 'YYYY-MM-DD'::text) AS to_date
   FROM ((public.films a
     JOIN public.film_attributes_vals b ON ((b.film_id = a.id)))
     JOIN public.film_attributes c ON ((c.id = b.attr_id)))
  WHERE ((c.type_id = 4) AND ((to_date(b.val, 'YYYY-MM-DD'::text) = ( SELECT to_date(settings.val, 'YYYY-MM-DD'::text) AS to_date
           FROM public.settings
          WHERE ((settings.name)::text = 'curdate'::text))) OR (to_date(b.val, 'YYYY-MM-DD'::text) = ( SELECT (to_date(settings.val, 'YYYY-MM-DD'::text) + '20 days'::interval)
           FROM public.settings
...
CREATE VIEW public.v_marketing AS
 SELECT f.id,
    ...
    fat.type,
        CASE
            WHEN ((fat.type)::text = 'int'::text) THEN ((fav.val)::integer)::text
            WHEN ((fat.type)::text = 'time'::text) THEN to_char((fav.val)::interval, 'HH24:MI:SS'::text)
            WHEN ((fat.type)::text = 'date'::text) THEN to_char(((fav.val)::date)::timestamp with time zone, 'dd.mm.yyyy'::text)
            WHEN ((fat.type)::text = 'float'::text) THEN ((fav.val)::double precision)::text
            WHEN ((fat.type)::text = 'checkbox'::text) THEN array_to_string(string_to_array(fav.val, '|'::text), ', '::text)
            ELSE fav.val
