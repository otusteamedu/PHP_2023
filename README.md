...
- Если cvv пуст, то после получения ответа, поле "CVV" подсвечивается красной рамкой
- Если поле cvv содержит больше 3 символовов, то после получения ответа, поле "CVV" подсвечивается красной рамкой

- Если order_number пуст, то после получения ответа, поле "Номер заказа" подсвечивается красной рамкой
- Если order_number содержит больше 16 символов, то после получения ответа, поле "Номер заказа" подсвечивается красной рамкой

- Если sum пуст, то после получения ответа, поле "Сумма" подсвечивается красной рамкой
- Если sum содержит больше 2 запятых, то после получения ответа, поле "Сумма" подсвечивается красной рамкой
