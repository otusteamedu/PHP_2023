...
from movie
         left join (
    select mav.movie_id, string_agg(concat(ma."name", ',', mav.value_date::text), '; ') as today from movie_attributes_value as mav
    ...
    group by mav.movie_id
         ...
         left join (
    select mav.movie_id, string_agg(concat(ma."name", ',', mav.value_date::text), '; ') as "+20d" from movie_attributes_value as mav
                                                                                                           LEFT JOIN movie_attributes ma ON mav.attribute_id = ma.id
    where
            mav.active = true and ma.parent_id  = 'c0f3fa24-7f39-4c8a-b92b-cdd9c43c0363' and ma.type_id = '64966228-bc4b-4216-b123-d10b6fa9fee1' and
            value_date >= current_date + interval '20 days'
    group by mav.movie_id
