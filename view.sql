create view today_task as
    select f.id as movie_id, a.name as task
    from film as f
...
create view movie_task as
    ...
    from movies as f
        inner join today_task t on f.id = t.movie_id
        inner join task_in_20_days t20 on f.id = t20.movie_id;

create view marketing_data as
    select f.name, t.name, a.name, v.int_value, v.float_value, v.bool_value, v.date_value, v.text_value
    from value v
        inner join movies f on f.id = v.movie_id
        inner join attribute a on a.id = v.attribute_id
        inner join type t on t.id = a.type_id