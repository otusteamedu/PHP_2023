SELECT count(tickets.id) FROM tickets LEFT JOIN sessions ON tickets.session_id = sessions.id
WHERE sessions.datetime BETWEEN date_trunc('week', now()::timestamp)::timestamp AND (date_trunc('week', now()::timestamp) + '6 days 23 hours 59 minutes'::interval)::timestamp
...
-- Анализ показал, что планировщик учел созданный тикет при построении запроса. Время выполнения запроса немного уменьшилось, однако индекс занял часть оперативной памяти
