...
class AppFixtures extends Fixture
{
    ...
    public function loadExpense(ObjectManager $manager): void
    {
        for ($id = 1; $id <= self::ENTITIES_COUNT; ++$id) {
            $expense = new Expense();
            $expense->setDate($this->faker->dateTimeBetween('-10 days'));
            $expense->setAmount((int)$this->faker->randomFloat(0, -100, -5000));
    ...
    public function loadIncome(ObjectManager $manager): void
    {
        for ($id = 1; $id <= self::ENTITIES_COUNT; ++$id) {
            $income = new Income();
            $income->setDate($this->faker->dateTimeBetween('-10 days'));
            $income->setAmount((int)$this->faker->randomFloat(0, 100, 5000));
