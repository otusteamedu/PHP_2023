...
CREATE OR REPLACE VIEW marketing_assembly AS SELECT f.name AS film, at.name AS attribute_type, a.name AS attribute, fa.char_value, fa.bool_value, fa.date_value FROM films AS f JOIN film_attribute_values AS fa ON f.id=fa.film_id JOIN attributes AS a ON fa.attribute_id=a.id JOIN attribute_types AS at ON at.id=a.attribute_type_id;
