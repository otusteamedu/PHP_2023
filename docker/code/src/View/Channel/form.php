...
?>
    ...
    <form action="" method="post">
        <div class="form-group">
            ...
            echo $formData['channel_id'] ?>"
                   required <?= (!empty($formData['id']) && $arrResult['formType'] !== 'add') ? "disabled" : '' ?> >
        </div>

        <div class="form-group">
            <label for="channel_name">Название канала:</label>
            <input type="text" id="channel_name" name="channel_name" value="<?php
            echo $formData['channel_name'] ?>" required>
        </div>

        <div class="form-group">
            <label for="subscribers">Подписчики:</label>
            <input type="text" id="subscribers" name="subscriber_count" value="<?php
            echo $formData['subscriber_count'] ?>" required>
        </div>
